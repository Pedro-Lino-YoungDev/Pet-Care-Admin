import Style from '../Style/ModificarCadastroDoUsuario.module.css'
import {useState, useEffect} from 'react'
import axios from 'axios';
import { Navigate} from 'react-router-dom';
import jwtDecode from 'jwt-decode'
import estilo from '../Style/User.module.css'



function AtualizarPerfil() {



    if (localStorage.getItem("token") != null) {

        const DataAtual = new Date();
        const HorarioTokenFormatado = parseInt(DataAtual.valueOf()/1000);

        const token_jwt = localStorage.getItem("token");
        const TokenDecodificado = jwtDecode(token_jwt);
        
        if (TokenDecodificado.exp > HorarioTokenFormatado) {

            const [animacao,setAnimacao] = useState(true);
            const [carregando,setCarregando] = useState(false);
            const [erroPost, setErroPost] = useState();
            const id = TokenDecodificado.id;

            const [dados, setDados] = useState({
                "name": "",
                "org": "",
                "email": "",
                "photo": "",
                "password": "",
                "newPassword": "",
                "token": token_jwt
            })

            useEffect(() => {
            axios.post("https://backend-petcare.herokuapp.com/admin/"+TokenDecodificado.id,{
            "token" : token_jwt
            })
            .then((res) => setDados((prev)=>{
                let NovosDados = {...prev, name: res.data.admin.name , org : res.data.admin.org, email: res.data.admin.email, photo: res.data.admin.photo}
                return NovosDados;
            }))
            .catch((res) => setErroPost((res.response.data.message)))
            .finally(() => setAnimacao(false))
            },[]);


            const [senhaVerificada, setSenhaVerificada] = useState();
            const [validador,setValidador] = useState(false);
            const [cancelar,setCancelar] = useState(false);

            const [resposta,setResposta] = useState();
            const [erro,setErro] = useState();
            const [chave, setChave] = useState("1");
            const [exibirErro, setExibir] = useState(false);
            
            const FotoGeral = "data:image/png;base64,"
            const validar = () =>{
                setValidador(!validador)
            }
            const put = () => {
                axios.put('https://backend-petcare.herokuapp.com/admin/'+id,dados)
                .then((res) => setResposta(res.data.message))
                .catch((res) => setErro(res.response.data.message))  
                .finally(() => setCarregando(false)) 
            }
            const editar_dados = (e) =>{
                if(e.target.id == "Nome"){
                    setDados((prev) =>{
                    let NovoNome = {...prev , name : e.target.value}
                    return NovoNome
                    })
                }
                else if(e.target.id == "Foto"){
                    if(e.target.files != undefined && e.target.files!= null){
                        setChave("1")
                        const imagemCarregada = e.target.files[0];
                        const arquivo = new FileReader ();
    
                        arquivo.onload = function (arquivoCarregado) {
                            const image= arquivoCarregado.target.result;
                            setDados((prev) =>{
                                let NovaFoto = {...prev , photo : image}
                                return NovaFoto
                                })
                        }
                        arquivo.readAsDataURL(imagemCarregada);
                    }

                }
                else if(e.target.id == "DeletarFoto"){
                    setChave("2")
                    setDados((prev) =>{
                        let SemFoto = {...prev , photo : FotoGeral}
                        return SemFoto
                        })
                }
                else if(e.target.id == "Senha"){
                    setDados((prev) =>{
                        let Senha = {...prev , password: e.target.value}
                        return Senha
                    })
                }
                else if(e.target.id == "NovaSenha"){
                    setDados((prev) =>{
                        let NovaSenha = {...prev , newPassword : e.target.value}
                        return NovaSenha
                    })
                }
                else if(e.target.id == "check"){
                    setSenhaVerificada("")
                    setDados((prev) =>{
                        let NovaSenha = {...prev , newPassword : ""}
                        return NovaSenha
                    })    
                }
            }
            while(animacao == true){
                return(
                    <div className={Style.ContainerMinimal}>
                        <div className={estilo.Carregamento}></div>
                    </div>
                )
            }
            const verificador = () =>{
                if(dados.name == null || dados.name == "" ){
                    return "Nome vazio"
                }
                else if(dados.name.length < 3){
                    return "Nome muito curto"
                }

                if(dados.password == null || dados.password == ""){
                    return "Campo senha vazio"
                }

                if(validador == true){
                    if(dados.newPassword == null || dados.newPassword == ""){
                        return "Nova senha invalida"
                    }
                    else if(dados.newPassword.length < 8){
                        return "Nova senha muito curta, digite uma senha com mais 7"
                    }
                    else if(dados.newPassword== "12345678" || dados.newPassword == "87654321" || 
                    dados.newPassword== "01234567" || dados.newPassword== "76543210" || dados.newPassword == "0123456789" || dados.newPassword == "123456789" || 
                    dados.newPassword == "012345678" || dados.newPassword == "987654321" || dados.newPassword =="9876543210" || dados.newPassword == "876543210"){
                        return "Senha com sequência númerica, digite uma mais segura"
                    }
                    else if(senhaVerificada != dados.newPassword || senhaVerificada == null){
                        return "Senhas não considem"
                    }
                    return "tudo ok"
                }
                    return "tudo ok" 
            }
            return(
                <div className={Style.ContainerMinimal}>
                    <form className={Style.form} action="Cadastro" method="Post" encType="multipart/form-data">
                        <div className={Style.ItemForm}>
                            <img className={Style.Imagem} src={dados.photo} alt="" />
                            <label className={Style.FileLabel} htmlFor="Foto"></label>
                            { dados.photo != null && dados.photo != FotoGeral &&(
                                <a className={Style.Logout} id="DeletarFoto" onClick={(e) => {editar_dados(e)}}>X</a>
                            )
                            }
                            <input className={Style.InputFile} key={chave} type="file" accept="image/*" name="image" id="Foto" onChange ={(e) => {editar_dados(e)}}/>
                        </div>
                        <div className={Style.ItemForm}>
                            <label className={Style.Label} htmlFor="Nome">Nome:</label>
                            <input className={Style.Input}  value={dados.name} type="Text" id= "Nome" placeholder= "Exemplo: Maria" onChange={(e) => {editar_dados(e)}}/>
                        </div>
                        {verificador() == "Nome vazio"&& exibirErro == true &&(
                            <p className={Style.erro}>
                                {"*"+verificador()}
                            </p>
                        )}
                        {verificador() == "Nome muito curto" && exibirErro == true &&(
                            <p className={Style.erro}>
                                {"*"+verificador()}
                            </p>
                        )}
                        <div className={Style.ItemForm}>
                            <label className={Style.Label} htmlFor="Org">Org:</label>
                            <input className={Style.Input} value={dados.org} type="Text" id= "Org" disabled/>
                        </div>
                        <div className={Style.ItemForm}>
                            <label className={Style.Label} htmlFor="Email">Email:</label>
                            <input className={Style.Input} value={dados.email} type="Text" id= "Email" disabled/>
                        </div>
                        <div className={Style.ItemForm}>
                            <label className={Style.Label} htmlFor="Senha">Senha Atual:</label>
                            <input className={Style.Input} value={dados.password} type="password" id= "Senha" onChange={(e) => {editar_dados(e)}}/>
                        </div>
                        {verificador() == "Campo senha vazio" && exibirErro == true &&(
                            <p className={Style.erro}>
                                {"*"+verificador()}
                            </p>
                        )}
                        <div>
                            <br />
                            <label className={Style.Label} htmlFor="check">Deseja Modificar a Senha?</label>
                            <input className={Style.Input} value={validador} type="checkbox" id= "check" checked={validador} onChange={(e) => { validar(), editar_dados(e)}}/>
                        </div>
                        {validador == true &&(
                        <div className={Style.ItemForm}>
                            <label className={Style.Label} htmlFor="Nome">Nova Senha:</label>
                            <input className={Style.Input} value = {dados.newPassword} type="password" id= "NovaSenha"onChange={(e) => {editar_dados(e)}}/>
                        </div>
                        )
                        }
                        {validador == true && verificador() == "Nova senha invalida" && exibirErro == true &&(
                            <p className={Style.erro}>
                                {"*"+verificador()}
                            </p>  
                        )}
                        {validador == true && verificador() == "Nova senha muito curta, digite uma senha com mais 7" && exibirErro == true &&(
                            <p className={Style.erro}>
                                {"*"+verificador()}
                            </p>  
                        )}
                        {validador == true && verificador() == "Senha com sequência númerica, digite uma mais segura" && exibirErro == true &&(
                            <p className={Style.erro}>
                                {"*"+verificador()}
                            </p>  
                        )}
                        {validador == true &&(
                        <div className={Style.ItemForm}>
                            <label className={Style.Label} htmlFor="Nome">Confirmar Nova Senha:</label>
                            <input className={Style.Input} value={senhaVerificada} type="password" id= "SenhaVerificada"onChange={(e) => {setSenhaVerificada(e.target.value)}}/>

                        </div>
                        )
                        }
                        {validador == true && verificador() == "Nova senha invalida" && exibirErro == true &&(
                            <p className={Style.erro}>
                                {"*"+verificador()}
                            </p>  
                            )}
                        {erro == "login attempt failed" &&(
                            <h4 className={Style.erro}>                
                                Oops! a sua senha está incorreta     
                            </h4>
                        )
                        }
                    </form>
                    {verificador() == "tudo ok" && carregando == false && (
                    <div className={Style.DivBtn}>
                        <a className={Style.Btn} onClick={() => {put(),setCarregando(true)}}>
                            Enviar
                        </a>
                    </div>
                    )}
                    {verificador() != "tudo ok" && carregando == false && (
                    <div className={Style.DivBtn}>
                        <a className={Style.Btn} onClick={() => setExibir(true)}>
                            Enviar
                        </a>
                    </div>
                    )}
                    {carregando == true&&(
                        <div className={estilo.Carregamento}></div>
                    )}
                    <div className={Style.DivBtn}>
                        <a className={Style.Btn} onClick={() => setCancelar(true)}>
                            Cancelar
                        </a>
                    </div>
                    {cancelar == true &&(
                        <Navigate to="/perfil"/>
                    )}
                    {resposta == "records updated successfully with new passowrd" && dados.photo != null&&(
                        localStorage.setItem("foto",dados.photo),
                        <Navigate to="/home"/>
                    )
                    }
                    {resposta == "records updated successfully with old password" && dados.photo != null &&(
                        localStorage.setItem("foto",dados.photo),
                        <Navigate to="/home"/>
                    )
                    }
                    {erro == "token has expired" && localStorage.removeItem("token")&&(
                        <Navigate to="/login"/>
                    )
                    }
                    {erroPost == "token has expired" && localStorage.removeItem("token")&&(
                        <Navigate to="/login"/>
                    )
                    }
                </div>
            )
        }
        else{
            return <Navigate to="/login" />
        }
    }
    else{
        return <Navigate to="/login" />
    }
}
export default AtualizarPerfil